...
(() => {
  ...
  function renderResult(rawScore) {
    const score = MindScoreUtils.clamp(Number(rawScore), 0, 100);
    ...
    animateValue({
      ...
      duration: 1300,
      onUpdate: (v) => {
        numberEl.textContent = v.toFixed(0);
        const offset = circumference - (v / 100) * circumference;
        bar.style.strokeDashoffset = offset;
      },
      onComplete: () => {
        numberEl.textContent = score.toFixed(2).replace(/\.00$/, '');
