...
const MindScoreAPI = (() => {
  ...
  async function predict(values) {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        throw new Error('The prediction server is waking up (free hosting sleeps when idle) and took too long. Please try again in a few seconds.');
      }
      throw new Error('Could not reach the prediction server. Please check your connection and try again.');
    }
    clearTimeout(timeoutId);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        const raw = errBody && (errBody.detail || errBody.message);
        if (typeof raw === 'string') {
          detail = raw;
        } else if (Array.isArray(raw)) {
          // FastAPI/pydantic 422 validation errors: array of {loc, msg, type}
          detail = raw
            .map((e) => {
  ...
  return { predict, buildPayload, ENDPOINT, BASE_URL };
})();
